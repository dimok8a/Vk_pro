...
<input type="checkbox" id="choicer">
<label for="choicer" class="lables">Щелканье</label>
</div>
<div class="choicer_div">
<input type="checkbox" id="choicer2">
<label for="choicer2" class="lables">Музыка из лифта</label>
</div>`;
document.querySelector('#side_bar_inner').appendChild(checkboxDiv);
const choicer2 = document.querySelector('#choicer2');



if (!localStorage.getItem('check')) { // Если в ls нет check, то создаем его со значением true по дефолту
    choicer.checked = true;
    localStorage.setItem('check', (choicer.checked).toString());
} else {
    choicer.checked = JSON.parse(localStorage.getItem('check')); // А иначе значение кнопки берем из ls (последнее выбранное пользователем)
}
if (!localStorage.getItem('check2')) { // Если в ls нет check, то создаем его со значением true по дефолту
    choicer2.checked = true;
    localStorage.setItem('check2', (choicer2.checked).toString());
} else {
    choicer2.checked = JSON.parse(localStorage.getItem('check2')); // А иначе значение кнопки берем из ls (последнее выбранное пользователем)
}

window.addEventListener('storage', () => {
    if (localStorage.getItem('check') == 'false' && choicer.checked == true) { // Если значение кнопки и ls разные, то делаем их одинаковыми (жмем на кнопку)
        choicer.click();
    }
    if (localStorage.getItem('check') == 'true' && choicer.checked == false) { // Тоже самое
        choicer.click();
    }
    if (localStorage.getItem('check2') == 'false' && choicer2.checked == true) { // Если значение кнопки и ls разные, то делаем их одинаковыми (жмем на кнопку)
        choicer2.click();
    }
    if (localStorage.getItem('check2') == 'true' && choicer2.checked == false) { // Тоже самое
        choicer2.click();
    }
});


choicer.addEventListener('change', () => { // Отслеживаем изменение кнопки
    localStorage.setItem('check', (choicer.checked).toString()); // Значение измененной кнопки записываем в ls
    if (choicer.checked) {
        clickSound('#top_notify_btn', bellSoundOpt); // Щелканье при клике на колокольчик  
        clickSound('#top_audio', bellSoundOpt); // Щелканье при клике на аудио  
        clickSound('#top_profile_link', profileSoundOpt); // Щелканье при клике на профиль в правом верхнем углу
    } else {
        stopClickSound('#top_notify_btn', bellSoundOpt);
        stopClickSound('#top_audio', bellSoundOpt);
        stopClickSound('#top_profile_link', profileSoundOpt);
    }
});
choicer2.addEventListener('change', () => { // Отслеживаем изменение кнопки
    localStorage.setItem('check2', (choicer2.checked).toString()); // Значение измененной кнопки записываем в ls
    voiceMessageSound();
});
...
try {
    var names = document.getElementsByClassName("TopNavBtn__profileName")[0];
...
const bellSoundOpt = function () {
    soundOpt('https://zvukipro.com/uploads/files/2019-09/1568276021_f946f3f8c39b945.mp3');
};

const audioSoundOpt = function () {
    soundOpt('https://zvukipro.com/uploads/files/2019-09/1568276021_f946f3f8c39b945.mp3');
};

const profileSoundOpt = function () {
    soundOpt('https://zvukipro.com/uploads/files/2019-09/1568276128_8bb9a29014590e5.mp3');
};

function clickSound(obj, func) { // Функция щелканья при клике на объект
    document.querySelector(obj).addEventListener('click', func);
}

function stopClickSound(obj, func) { // Удаляем слушатель выше
    document.querySelector(obj).removeEventListener('click', func);
}
...
function clickAndNavSound(div, overS, clickS, overSVol = .1, clickSVol = .3) { // (большой объект, звук при наведении на маленький, звук при нажатии на маленький, громкость звука при наведении, громкость звука при нажатии)
    for (let i = 0; i < document.querySelectorAll(div).length; i++) { //div - большой объект, в котором есть много маленьких
        ...
        document.querySelectorAll(div)[i].addEventListener('click', () => {
            ...
            clickSound.autoplay = true;
            document.querySelectorAll(div)[i].classList.add('grey__bgc');
            setTimeout(() => {
                document.querySelectorAll(div)[i].classList.remove('grey__bgc');
            }, 400);
        });
    }
}
...
const audio = new Audio('https://zvukipro.com/uploads/files/2019-07/1564068640_cf9b99726102246.mp3'); // Звук печати
audio.autoplay = false;
audio.volume = 0.3;


const audio2 = new Audio('https://zvukipro.com/uploads/files/2020-12/1609052060_elevator-music-vanoss-gaming-background-music.mp3'); // Звук при записи голосовых
audio2.autoplay = false;
audio2.volume = 0.3;

function voiceMessageSound() {
    setInterval(() => {
        try {
...
const newBgc = (block, actClass, newClass = 'green__bgc') => { // Функция, которая меняет цвет фона блоку, если у него есть определенный класс
    setInterval(() => {
        if (document.querySelector(block).classList.contains(actClass)) {
            document.querySelector(block).classList.add(newClass);
        } else {
            document.querySelector(block).classList.remove(newClass);
        }
    }, 100);
};
...
// * Неперетаскиваемые стикеры
setInterval(function () {
    if (document.querySelectorAll('.im_gift').length !== 0) {
        let stickers = document.querySelectorAll('.im_gift');
